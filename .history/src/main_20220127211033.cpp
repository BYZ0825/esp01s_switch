...
#define RELAYGPIO0 0 //GPIO 0
#define RELAYGPIO2 2 //GPIO 2
#define RELAYGPIO1 1 //TX GPIO1
#define RELAYGPIO3 3 //RX GPIO3

uint8_t SW0 = RELAYGPIO0;
uint8_t SW1 = RELAYGPIO1;
uint8_t SW2 = RELAYGPIO2;
uint8_t SW3 = RELAYGPIO3;

BlinkerButton Button0("btn-0"); //注意：要和APP组件’数据键名’一致
BlinkerButton Button1("btn-1"); //注意：要和APP组件’数据键名’一致
BlinkerButton Button2("btn-2"); //注意：要和APP组件’数据键名’一致
BlinkerButton Button3("btn-3"); //注意：要和APP组件’数据键名’一致

bool wsState[5] = {false};
...
{
    BLINKER_LOG("need set power state: ", state, "num_back", num);
    if (num ==)
