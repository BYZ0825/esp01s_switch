...
uint8_t SW1 = RELAYGPIO0;
uint8_t SW2 = RELAYGPIO1;
uint8_t SW3 = RELAYGPIO2;
uint8_t SW4 = RELAYGPIO3;

BlinkerButton Button1("btn-0"); //注意：要和APP组件’数据键名’一致
BlinkerButton Button1("btn-1"); //注意：要和APP组件’数据键名’一致
BlinkerButton Button1("btn-2"); //注意：要和APP组件’数据键名’一致
BlinkerButton Button1("btn-3"); //注意：要和APP组件’数据键名’一致
...
{
    BLINKER_LOG("get button state: ", state);
    digitalWrite(SW4, !digitalRead(SW4));
    Blinker.vibrate();
}
//小爱电源类操作的回调函数:
